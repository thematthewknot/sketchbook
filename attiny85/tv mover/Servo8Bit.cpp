...
void ServoSequencer::servoTimerSetup()
{
    if( timerIsSetup == false )
    {
        ...
        //clear register because the attiny cores sets bit in it which we dont want set
        TCCR1 = 0;
        ...
        #else
            //if its not 8mhz, assume it is 1mhz.
...
void ServoSequencer::timerCompareMatchISR()
{
    ...
    case WAITING_TO_SET_PIN_HIGH:
        ...
        OCR1A = 64 - 4; //trim off 4 ticks (32us), this is about the total combined time we spent inside this ISR;
        //update our state
    ...
    case WAITING_FOR_512_MARK:
        ...
        OCR1A = servoRegistry[servoIndex].pulseLengthInTicks;
        //update our state
        ...
        if(OCR1A == 0)
        {
           ...
           TIFR &= ~(1 << OCF1A);  // write 0 to the OCF0A flag to set it
    ...
    case WAITING_TO_SET_PIN_LOW:
        ...
        if( (64 + servoRegistry[servoIndex].pulseLengthInTicks) > 255 )
        {
            ...
            OCR1A = 512 - (64 + servoRegistry[servoIndex].pulseLengthInTicks);
        }
        else
        {
            ...
            OCR1A = 255 - (64 + servoRegistry[servoIndex].pulseLengthInTicks);
        }
